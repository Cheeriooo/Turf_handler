...
const MatchSetup: React.FC<MatchSetupProps> = ({ onMatchStart, onShowHistory }) => {
  ...
  const TeamSection = ({ team, name, setName, players }: any) => (
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
        {players.map((player: string, index: number) => (
          <div key={index} className="flex items-center gap-2 group">
            ...
            <input
              type="text" value={player} onChange={(e) => handlePlayerChange(team, index, e.target.value)}
            ...
            />
            <button type="button" onClick={() => removePlayer(team, index)} disabled={players.length <= 2} className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-400 transition p-1">
              <CloseIcon className="w-4 h-4" />
            </button>
          </div>
        ))}
      </div>
      <button type="button" onClick={() => addPlayer(team)} disabled={players.length >= 11} className="w-full py-2 text-sm font-semibold text-gray-400 border border-dashed border-gray-700 rounded-lg hover:bg-white/5 hover:text-white transition disabled:opacity-30">
        + Add Player
      </button>
    </div>
  );
      ...
      <div className="max-w-4xl w-full animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <TeamSection team="team1" name={team1Name} setName={setTeam1Name} players={team1Players} />
             <TeamSection team="team2" name={team2Name} setName={setTeam2Name} players={team2Players} />
          </div>
